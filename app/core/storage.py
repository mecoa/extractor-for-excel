...
class ResultCache:
    ...
    def save_field(self, row_index: int, field_name: str, value: str, confidence: str = "missing", llm_reasoning: str = ""):
        self._conn.execute(
            """INSERT OR REPLACE INTO extract_results
               (row_index, field_name, value, confidence, llm_reasoning, updated_at)
               VALUES (?, ?, ?, ?, ?, CURRENT_TIMESTAMP)""",
            (row_index, field_name, value, confidence, llm_reasoning),
        )
        self._conn.commit()

    def save_all(self, results: Dict[int, dict]):
        for row_idx, data in results.items():
            for field_name, fd in data.items():
                if field_name == "_error":
                    continue
                self.save_field(
                    row_idx, field_name,
                    fd.get("value", ""),
                    fd.get("confidence", "missing"),
                    fd.get("llm_reasoning", ""),
                )

    def load_all(self) -> Dict[int, dict]:
        rows = self._conn.execute(
            "SELECT * FROM extract_results ORDER BY row_index, field_name"
        ).fetchall()
        results: Dict[int, dict] = {}
        for row in rows:
            ridx = row["row_index"]
            if ridx not in results:
                results[ridx] = {}
            results[ridx][row["field_name"]] = {
                "value": row["value"],
    ...
    def clear(self):
        self._conn.execute("DELETE FROM extract_results")
        self._conn.commit()
